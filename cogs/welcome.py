...
class WelcomeCmds(commands.Cog):
    ...
    welcome = app_commands.Group(name="welcome",description="Welcome Commands",guild_only=True)    

    @welcome.command(name="channel",description="Set the channel for welcome")
    ...
    async def set_message(self,interaction:discord.Interaction,*,message: str):
        ...
        if not check:
            return await interaction.response.send_message("Please set a channel first")
        await self.bot.db.execute(
            "UPDATE welcome SET response = $1 WHERE guild_id = $2",
    ...
    async def test(self,interaction:discord.Interaction,user:discord.Member):
        ...
        await interaction.response.send_message("Sent test message")
